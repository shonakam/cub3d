...
static void	calculate_ray_steps(t_cub3d *cub, t_ray *ray)
{
	...
	{
		ray->step.x = -1;
		ray->side_distance.x = (cub->player.position.x - ray->map.x) * ray->delta_distance.x;
	}
	else
	{
		ray->step.x = 1;
		ray->side_distance.x = (ray->map.x + 1.0 - cub->player.position.x) * ray->delta_distance.x;
	}
	if (ray->direction.y < 0)
	{
		ray->step.y = -1;
		ray->side_distance.y = (cub->player.position.y - ray->map.y) * ray->delta_distance.y;
	}
	else
	{
		ray->step.y = 1;
		ray->side_distance.y = (ray->map.y + 1.0 - cub->player.position.y) * ray->delta_distance.y;
	}
}
